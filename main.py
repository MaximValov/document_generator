...
def add_image_to_cell(cell, image_path, width_cm, height_cm=None, filename=None):
...
def create_image_table_doc(image_files, table_rows, table_cols, width_cm, height_cm=None):
    ...
    # Distribute images evenly in the table with filenames
    img_count = 0
    for i in range(table_rows):
        for j in range(table_cols):
            if img_count < len(image_files):
                ...
                add_image_to_cell(cell, tmp_path, width_cm, height_cm, filename)
...
def create_image_table_preview(image_files, table_rows, table_cols, width_cm, height_cm=None):
    """Create a properly working HTML preview of the image table"""
    # Create a container for our preview
    preview_container = st.container()

    # Use columns to create a table-like structure
...
def main():
    ...
    # Create tabs for separate functionality
            ...
            if st.button("Preview Image Table", key="preview_img_table"):
                with st.spinner("Generating preview..."):
                    try:
                        ...
                        preview_html = create_image_table_preview(
                            image_files,
                            table_rows,
                            table_cols,
                            width_cm,
                        ...
                        st.info(
                            "Note: This is an approximation of how the table will look in Word. Filenames (without extensions) are shown below each image.")
